...
fn main() {
    ...
    if args.len() != 2 {
        println!("Minimizes and hides all windows belonging to the given process ID.");
        println!("Usage: blb.exe <pid>");
        process::exit(0);
    }
    ...
    unsafe {
        ...
        if WINDOW_MATCHED {
            println!("Successfully minimized and hid windows for PID {}.", pid);
            process::exit(0);
        } else {
